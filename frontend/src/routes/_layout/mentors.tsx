import {
  ...
  import { MentorsService, type UserPublic } from "../../client"
  ...
  export const Route = createFileRoute("/mentors")({ // Correct path
    component: Mentors,
  ...
  function MentorsTable() {
    ...
    return (
      <>
        <TableContainer>
          <Table size={{ base: "sm", md: "md" }}>
            ...
            {isPending ? (
              ...
              <Tbody>
                {mentors?.map((mentor: UserPublic, index: number) => (
                  <Tr key={index} opacity={isPlaceholderData ? 0.5 : 1}>
                    <Td>{mentor.email}</Td>
                    <Td>
                      <ActionsMenu type={"Mentor"} value={mentor} /> 
                    </Td>
                  </Tr>
                ))}
              </Tbody>
        ...
        >
          <Button onClick={() => navigate({ to: "/mentors", search: { page: page - 1 } })} isDisabled={page <= 1}> 
            Previous
          </Button>
          <span>Page {page}</span>
          <Button isDisabled={!hasNextPage} onClick={() => navigate({ to: "/mentors", search: { page: page + 1 } })}> 
            Next
          </Button>
        </Flex>
      </>
