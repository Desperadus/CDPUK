...
# New routes for mentors and questionnaires
@router.post("/me/mentors/{mentor_id}", response_model=Mentor)
def assign_mentor(
    *,
    session: SessionDep,
    mentor_id: uuid.UUID,
    ...
    mentor = crud.create_mentor(session=session, mentee_id=current_user.id, mentor_id=mentor_id)
    return mentor

@router.post("/me/questionnaires", response_model=Questionnaire)
...
@router.get("/me/questionnaires", response_model=list[Questionnaire])
